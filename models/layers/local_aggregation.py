...
class ASSA(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, features, query_idx=None):
        ...
        features_float = self.dequant_feat(features)
        # grouping 
        dp, fj = self.grouper(query_xyz, support_xyz, features_float)
        if self.use_res and query_idx is not None:
            features_float = torch.gather(
                features_float, -1, query_idx.unsqueeze(1).expand(-1, features_float.shape[1], -1))
        ...
        if self.use_res:
            out_features = self.act(out_features + self.skip_layer(features_float))
        return out_features
    ...
    def forward(self, query_xyz, support_xyz, features, query_idx=None):
        ...
        if 'df' in self.feature_type or self.use_res:
            ...
            if self.use_res and neighbor_dim != 2:
                identity = self.skipconv(features_float)
            else:
                identity = 0
        ...
        # """End of debug"""
        fj = get_aggregation_feautres(query_xyz, dp, features_float, fj, feature_type=self.feature_type)
        fj = self.quant_feat(fj)
