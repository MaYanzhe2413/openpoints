...
_kdtree_leaf_fps_sample_logged = False
...
def kdtree_leaf_fps_sample(xyz: torch.Tensor, npoint: int, 
    ...
    Returns:
        idx: (B, npoint) - 采样索引
    """
    global _kdtree_leaf_fps_sample_logged
    device = xyz.device
    B, N, C = xyz.shape
    npoint = min(npoint, N)
    if not _kdtree_leaf_fps_sample_logged:
        logging.info(f"[Sampler] kdtree_leaf_fps_sample | B={B}, N={N}, npoint={npoint}, leaf_size={leaf_size} | sklearn.KDTree + leaf-internal FPS")
        _kdtree_leaf_fps_sample_logged = True
...
_kdtree_leaf_simple_sample_logged = False
...
f.create_prefetch_pointmap()
    ...
    Returns:
        idx: (B, npoint) 采样索引
    """
    global _kdtree_leaf_simple_sample_logged
    device = xyz.device
    B, N, C = xyz.shape
    npoint = min(npoint, N)
    if not _kdtree_leaf_simple_sample_logged:
        logging.info(f"[Sampler] kdtree_leaf_simple_sample | B={B}, N={N}, npoint={npoint}, leaf_size={leaf_size}, strategy={strategy}, proportional={proportional} | recursive median-split + leaf {strategy} sampling")
        _kdtree_leaf_simple_sample_logged = True
...
def kdtree_simple_sample(xyz: torch.Tensor, npoint: int,
    ...
    if not _kdtree_simple_sample_logged:
        logging.info(f"[Sampler] kdtree_simple_sample | B={B}, N={N}, npoint={npoint}, leaf_size={leaf_size}, strategy={strategy}, proportional={proportional} | -> kdtree_leaf_simple_sample")
...
_kdtree_vectorized_logged = False
...
def kdtree_simple_sample_vectorized(xyz: torch.Tensor, npoint: int,
    ...
    Returns:
        idx: (B, npoint) 采样索引
    """
    global _kdtree_vectorized_logged
    B, N, C = xyz.shape
    device = xyz.device
    npoint = min(npoint, N)
    if not _kdtree_vectorized_logged:
        logging.info(f"[Sampler] kdtree_simple_sample_vectorized | B={B}, N={N}, npoint={npoint}, leaf_size={leaf_size}, proportional={proportional} | GPU vectorized KDTree")
        _kdtree_vectorized_logged = True
    strategy = (strategy or 'random').lower()

    # -- KD-tree block splitting (per-block median, correct structure) --
    # block_id[b, n] = which block each point belongs to
    # All points start in block 0. Each split creates a new block for the right half.
    max_depth = max(1, int(np.ceil(np.log2(max(1, N / leaf_size)))))
    ...
    for depth in range(max_depth):
        ...
        for b in range(B):
            # Find all blocks in this batch that still need splitting
            unique_blocks = node_assignment[b].unique()
            for bid in unique_blocks:
                mask = (node_assignment[b] == bid)
                count = mask.sum().item()
                if count <= leaf_size:
                    continue  # already leaf-sized, skip

                # Compute median within THIS block only
                pts = xyz[b, mask, split_dim]
                med = pts.median()

                # Split: left keeps current bid, right gets new bid
                right_mask = mask & (xyz[b, :, split_dim] >= med)
                left_count = count - right_mask.sum().item()
                right_count = right_mask.sum().item()

                if left_count == 0 or right_count == 0:
                    continue  # degenerate split, treat as leaf

                node_assignment[b, right_mask] = next_block_id
                next_block_id += 1
                any_split = True
...
def kdtree_adaptive_leaf_fps_sample(xyz: torch.Tensor, npoint: int, 
    ...
    Returns:
        idx: (B, npoint) - 采样索引
    """
    global _kdtree_adaptive_logged
    device = xyz.device
    B, N, C = xyz.shape
    npoint = min(npoint, N)
    if not _kdtree_adaptive_logged:
        logging.info(f"[Sampler] kdtree_adaptive_leaf_fps_sample | B={B}, N={N}, npoint={npoint}, leaf_size={leaf_size}, density_adaptive={density_adaptive} | sklearn.KDTree + adaptive leaf FPS")
        _kdtree_adaptive_logged = True
...
# 简化接口，兼容PointNeXt
_kdtree_leaf_fps_simple_logged = False

def kdtree_leaf_fps_simple(xyz: torch.Tensor, npoint: int) -> torch.Tensor:
    """简化的KDTree叶节点FPS接口"""
    global _kdtree_leaf_fps_simple_logged
    if not _kdtree_leaf_fps_simple_logged:
        B, N, _ = xyz.shape
        logging.info(f"[Sampler] kdtree_leaf_fps_simple | B={B}, N={N}, npoint={npoint} | -> kdtree_leaf_fps_sample")
        _kdtree_leaf_fps_simple_logged = True
