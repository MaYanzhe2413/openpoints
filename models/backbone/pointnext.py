...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf) -> torch.Tensor:
        ...
        p, f = pf
        # --- quant boundary: dequantize features for geometry ---
        f_float = self.dequant_feat(f)
        # neighborhood_features
        dp, fj = self.grouper(p, p, f_float)
        fj = get_aggregation_feautres(p, dp, f_float, fj, self.feature_type)
        fj = self.quant_feat(fj)
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_res:
            ...
            self.quant_skip = quant.QuantStub()
        ...
        if not is_head:
            ...
            elif sampler.lower() in ('kdtree', 'kdtree_adaptive', 'kdtree_simple',
                                        'kdtree_vectorized', 'kdtree_simple_vectorized'):
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if self.use_res or 'df' in self.feature_type:
                    ...
                    f_float, -1, idx.unsqueeze(1).expand(-1, f_float.shape[1], -1))
                if self.use_res:
                    identity = self.skipconv(self.quant_skip(fi))
            else:
                fi = None
            dp, fj = self.grouper(new_p, p, f_float)
...
class FeaturePropogation(nn.Module):
    ...
    def __init__(self, mlp,
        ...
        self.dequant_f2 = quant.DeQuantStub()
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # sampler can be str or list[str]; sampler_args can be dict or list[dict]
        _sampler = kwargs.get('sampler', 'fps')
        _sampler_args = kwargs.get('sampler_args', {})
        n_stages = len(blocks)
        if isinstance(_sampler, list):
            assert len(_sampler) == n_stages, f"sampler list length {len(_sampler)} != stages {n_stages}"
            self.sampler_list = _sampler
        else:
            self.sampler_list = [_sampler] * n_stages
        if isinstance(_sampler_args, list):
            assert len(_sampler_args) == n_stages, f"sampler_args list length {len(_sampler_args)} != stages {n_stages}"
            self.sampler_args_list = _sampler_args
        else:
            self.sampler_args_list = [_sampler_args] * n_stages
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                ...
                is_head=i == 0 and strides[i] == 1, stage_idx=i
            ))
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False, stage_idx=0):
        layers = []
